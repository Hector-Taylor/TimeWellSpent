...
export class LibraryService extends EventEmitter {
  ...
  constructor(private database: Database) {
    ...
    this.listStmt = this.db.prepare(
      'SELECT id, sync_id, kind, url, app, domain, title, note, bucket, purpose, price, created_at, updated_at, last_used_at, consumed_at, deleted_at FROM library_items WHERE deleted_at IS NULL ORDER BY created_at DESC'
    );
    this.getByIdStmt = this.db.prepare(
      'SELECT id, sync_id, kind, url, app, domain, title, note, bucket, purpose, price, created_at, updated_at, last_used_at, consumed_at, deleted_at FROM library_items WHERE id = ? AND deleted_at IS NULL'
    );
    this.getByUrlStmt = this.db.prepare(
      'SELECT id, sync_id, kind, url, app, domain, title, note, bucket, purpose, price, created_at, updated_at, last_used_at, consumed_at, deleted_at FROM library_items WHERE url = ? AND deleted_at IS NULL'
    );
    this.getAnyByUrlStmt = this.db.prepare(
      'SELECT id, sync_id, kind, url, app, domain, title, note, bucket, purpose, price, created_at, updated_at, last_used_at, consumed_at, deleted_at FROM library_items WHERE url = ?'
    );
    this.insertStmt = this.db.prepare(
      'INSERT INTO library_items(sync_id, kind, url, app, domain, title, note, bucket, purpose, price, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    );
    ...
    this.updateStmt = this.db.prepare('UPDATE library_items SET title = ?, note = ?, bucket = ?, purpose = ?, price = ?, consumed_at = ?, updated_at = ? WHERE id = ?');
    ...
    this.reviveStmt = this.db.prepare(
      'UPDATE library_items SET title = ?, note = ?, bucket = ?, purpose = ?, price = ?, consumed_at = ?, deleted_at = NULL, updated_at = ? WHERE id = ?'
    );
    this.updateSyncStmt = this.db.prepare('UPDATE library_items SET sync_id = ? WHERE id = ?');
    this.upsertSyncStmt = this.db.prepare(
      `INSERT INTO library_items(sync_id, kind, url, app, domain, title, note, bucket, purpose, price, created_at, updated_at, last_used_at, consumed_at, deleted_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    this.listSinceStmt = this.db.prepare(
      'SELECT id, sync_id, kind, url, app, domain, title, note, bucket, purpose, price, created_at, updated_at, last_used_at, consumed_at, deleted_at FROM library_items WHERE updated_at >= ? ORDER BY updated_at ASC'
    );
    this.findByBaseUrlStmt = this.db.prepare(
      "SELECT id, sync_id, kind, url, app, domain, title, note, bucket, purpose, price, created_at, updated_at, last_used_at, consumed_at, deleted_at FROM library_items WHERE url = ? AND price IS NOT NULL AND deleted_at IS NULL LIMIT 1"
    );
  }
  ...
  add(payload: { kind: 'url' | 'app'; url?: string; app?: string; title?: string; note?: string; purpose: LibraryPurpose; price?: number | null }): LibraryItem {
    ...
    if (payload.kind === 'url') {
      ...
      const result = this.insertStmt.run(syncId, 'url', url, null, domain, payload.title ?? null, payload.note ?? null, bucket, purpose, price, now, now);
    ...
    const now = new Date().toISOString();
    const syncId = randomUUID();
    const result = this.insertStmt.run(syncId, 'app', null, app, app, payload.title ?? null, payload.note ?? null, bucket, purpose, null, now, now);
    const item: LibraryItem = {
      id: Number(result.lastInsertRowid),
      syncId,
      kind: 'app',
  ...
  update(
    ...
    const now = new Date().toISOString();
    this.updateStmt.run(nextTitle ?? null, nextNote ?? null, nextBucket, nextPurpose, nextPrice, nextConsumedAt, now, id);
    const item: LibraryItem = {
      ...existing,
