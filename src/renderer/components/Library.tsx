...
import type { LibraryItem, LibraryPurpose, RendererApi } from '@shared/types';
...
function purposeTone(purpose: LibraryPurpose) {
  if (purpose === 'replace') return 'productive';
          ...
          {visible.map((item) => (
            <div key={item.id} className="card store-item">
              ...
              <div className="store-item-meta">
                ...
                {item.lastUsedAt ? <span>Last used {formatDate(item.lastUsedAt)}</span> : <span>Not used yet</span>}
              </div>
